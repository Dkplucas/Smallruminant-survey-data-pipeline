# Young male Sahelian animals older than 6 months vs geographical zones
# Inputs: data7.xlsx and zones.xlsx
# Output: young_male_sahelian_over6months_zones_monte_carlo_results.xlsx
...
OUTPUT_FILE <- "young_male_sahelian_over6months_zones_monte_carlo_results.xlsx"
ALPHA <- 0.05
B <- 10000L
SEED <- 20260916L

COUNT_COL <- "12.) Effectif et composition du cheptel/Nbre de jeunes (>6 mois) mâles_Sahelien"
...
COUNT_COL <- resolve_column(
  COUNT_COL, names(raw),
  c("Effectif et composition du cheptel", "jeunes", "6 mois", "mâles", "Sahelien"),
  "Young male Sahelian count"
)
...
veg <- run_monte_carlo("vegetation_zone", "Young male Sahelian count (>6 months) x vegetation zone", 0L)
phyto <- run_monte_carlo("phytogeo_zone", "Young male Sahelian count (>6 months) x phytogeographic zone", 100L)
...
notes <- data.frame(
  ...
  Assessment = c(
    ...
    "Zero is valid and means no young male Sahelian animals older than six months.",
