# Monte Carlo permutation tests for adult male Djallonke count vs vegetation and phytogeographic zones
# Inputs: data7.xlsx and zones.xlsx
# Output: adult_male_djallonke_count_zones_monte_carlo_results.xlsx
...
OUTPUT_FILE <- "adult_male_djallonke_count_zones_monte_carlo_results.xlsx"
ALPHA <- 0.05
B <- 10000L  # Increase to 100000L for the final analysis if desired
SEED <- 20260916L

COUNT_COL <- "12.) Effectif et composition du cheptel/Nombre de mâle adultes Djallonke"
...
COUNT_COL <- resolve_column(COUNT_COL, names(raw), c("Effectif et composition du cheptel", "Nombre de male adultes", "Djallonke"), "Adult male Djallonke count")
...
data <- raw |>
  transmute(
    ...
    adult_male_djallonke_count = parse_number_safe(.data[[COUNT_COL]])
  ) |>
  left_join(zones, by = "commune_key") |>
  mutate(
    invalid_numeric = !is.na(count_raw) & str_squish(count_raw) != "" & is.na(adult_male_djallonke_count),
    negative_value = !is.na(adult_male_djallonke_count) & adult_male_djallonke_count < 0,
    noninteger_value = !is.na(adult_male_djallonke_count) & adult_male_djallonke_count >= 0 & abs(adult_male_djallonke_count - round(adult_male_djallonke_count)) > 1e-8,
    valid_count = ifelse(!is.na(adult_male_djallonke_count) & adult_male_djallonke_count >= 0 & !noninteger_value, adult_male_djallonke_count, NA_real_)
  )

describe_by_zone <- function(zone_var) {
  data |>
    filter(!is.na(valid_count), !is.na(.data[[zone_var]]), .data[[zone_var]] != "") |>
    group_by(Zone = .data[[zone_var]]) |>
    summarise(
      N = n(), Mean = mean(valid_count), SD = sd(valid_count),
      Median = median(valid_count), Q1 = quantile(valid_count, .25),
      Q3 = quantile(valid_count, .75), Minimum = min(valid_count),
      Maximum = max(valid_count), .groups = "drop"
    )
}
...
run_tests <- function(zone_var, label) {
  d <- data |>
    filter(!is.na(valid_count), !is.na(.data[[zone_var]]), .data[[zone_var]] != "") |>
    transmute(value = valid_count, zone = factor(.data[[zone_var]])) |>
    droplevels()
  ...
  if (n == 0L || k < 2L || length(unique(d$value)) < 2L || n <= k) {
    reason <- if (n == 0L) "No valid complete observations." else if (k < 2L) "Only one geographical zone is represented." else if (n <= k) "Too few observations for the number of groups." else "All valid adult male Djallonke counts are identical."
    return(list(
      summary = data.frame(
  ...
  summary <- data.frame(
    ...
    Decision = ifelse(monte_carlo_p < ALPHA, "Reject H0: adult male Djallonke count distributions differ across zones", "Do not reject H0: no distributional difference detected"),
...
veg <- run_tests("vegetation_zone", "Adult male Djallonke count x vegetation zone")
phyto <- run_tests("phytogeo_zone", "Adult male Djallonke count x phytogeographic zone")

quality <- data.frame(
  Metric = c("Total records", "Valid nonnegative integer counts", "Zero counts", "Positive counts", "Blank count responses", "Invalid numeric values", "Negative values", "Non-integer values", "Unmatched communes"),
  Value = c(
    nrow(data),
    sum(!is.na(data$valid_count)),
    sum(data$valid_count == 0, na.rm = TRUE),
    sum(data$valid_count > 0, na.rm = TRUE),
    sum(is.na(data$count_raw) | str_squish(data$count_raw) == ""),
    sum(data$invalid_numeric),
    sum(data$negative_value),
    sum(data$noninteger_value),
...
notes <- data.frame(
  Parameter = c("Variable type", "Primary analysis", "Permutation principle", "Number of simulations", "P-value correction", "Effect size", "Zero counts", "Validity rule"),
  Assessment = c(
    "Quantitative discrete count variable.",
    "Monte Carlo permutation test using the Kruskal-Wallis H statistic.",
    "Observed counts are randomly reassigned to existing zone memberships under the null hypothesis of identical distributions.",
    "10,000 random permutations are used by default for each zone analysis; B can be increased to 100,000 for the final run.",
    "The Monte Carlo p-value is calculated as (extreme permutations + 1)/(B + 1).",
    "Epsilon-squared based on the observed Kruskal-Wallis statistic.",
    "Zero is retained as a valid count meaning no adult male Djallonke animals.",
    "Only nonnegative integer counts with matched zones are analyzed; blanks, nonnumeric, negative, and noninteger values are excluded and reported."
  ), stringsAsFactors = FALSE
)
