#!/usr/bin/env Rscript
# Monte carlo chisq.test(..., simulate.p.value = TRUE, B = 20000) for Niveau d'instruction vs zones
# (standard Chi-square approximation was unreliable due to low expected counts)
...
run_monte_carlo_test <- function(data, zone_col, output_file, workbook) {
  ...
  cat("\n=== Monte Carlo test: Niveau d'instruction vs", zone_col, "===\n")
  print(tab)
  ...
  observed_df <- cbind(EducationLevel = rownames(observed_df), observed_df)
  rownames(observed_df) <- NULL

  expected_df <- as.data.frame.matrix(round(test$expected, 2))
  expected_df <- cbind(EducationLevel = rownames(expected_df), expected_df)
  rownames(expected_df) <- NULL

  summary_lines <- c(
    paste0("=== Monte Carlo test: Niveau d'instruction vs ", zone_col, " ==="),
  ...
  writeData(workbook, sheet = observed_sheet, x = cbind(Comparison = paste("Niveau d'instruction vs", zone_col), observed_df), startRow = 1, startCol = 1)
  writeData(workbook, sheet = expected_sheet, x = cbind(Comparison = paste("Niveau d'instruction vs", zone_col), expected_df), startRow = 1, startCol = 1)
...
# NOTE: source of truth is Data_cleaning/data7.xlsx; local copies in other folders can be stale.
data_path <- "C:/Users/lucas/OneDrive/Bureau/Data/Data_cleaning/data7.xlsx"
zones_path <- file.path(base_dir, "zones.xlsx")
output_file <- file.path(base_dir, "education_zone_monte_carlo_results.xlsx")
if (file.exists(output_file)) {
  output_file <- file.path(base_dir, paste0("education_zone_monte_carlo_results_", format(Sys.time(), "%Y%m%d_%H%M%S"), ".xlsx"))
}
