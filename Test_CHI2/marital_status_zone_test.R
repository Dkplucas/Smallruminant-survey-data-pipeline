...
survey_file <- file.path(base_dir, "data8.xlsx")
zones_file <- file.path(base_dir, "zones.xlsx")
output_file <- file.path(base_dir, "marital_status_zone_chisq_results.xlsx")
...
marital_col <- find_column(survey_raw, "Situation matrimoniale")
...
survey <- survey_raw %>%
  select(all_of(marital_col), all_of(commune_col)) %>%
  rename(MaritalStatus = !!marital_col, Commune = !!commune_col) %>%
  mutate(
    MaritalStatus = as.character(MaritalStatus),
    Commune = as.character(Commune),
    MaritalStatus = str_trim(MaritalStatus)
  )
...
run_test <- function(data, zone_col) {
  ...
  data <- data %>% filter(!is.na(!!zone_sym), !is.na(MaritalStatus))
  tab <- table(data$MaritalStatus, data[[zone_col]])
...
write_block <- function(sheet, result, start_row) {
  ...
  obs_df <- cbind(MaritalStatus = rownames(obs_df), obs_df)
  rownames(obs_df) <- NULL
  writeData(wb, sheet = sheet, x = data.frame(Comparison = paste("Marital status vs", result$zone_col)), startRow = start_row, startCol = 1)
...
write_block_expected <- function(sheet, result, start_row) {
  ...
  exp_df <- cbind(MaritalStatus = rownames(exp_df), exp_df)
...
summary_text <- c(
  paste0("=== Chi-square test: Marital status vs VegetationZone ==="),
  paste0("Chi-square statistic: ", results$veg$statistic),
  paste0("Chi-square p-value: ", format(results$veg$p_value, nsmall = 4)),
  "",
  paste0("=== Chi-square test: Marital status vs PhytogeographicZone ==="),
