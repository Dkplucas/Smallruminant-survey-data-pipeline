# Djallonke more mortality cases - Non modality vs vegetation and phytogeographic zones
# Inputs: data7.xlsx and zones.xlsx
# Output: chi_square_results_djallonke_more_mortality_cases_no_zones.xlsx
...
OUTPUT_FILE <- "chi_square_results_djallonke_more_mortality_cases_no_zones.xlsx"
ALPHA <- 0.05
B <- 100000L
SEED <- 20260915L
...
CHILD_COL <- paste0(
  ...
  "que chez les métis et autres races dans votre élevage ? /Non"
)
...
resolve_column <- function(expected, headers, required_terms, label, candidate_pattern) {
  ...
  candidates <- headers[str_detect(normalized_headers, candidate_pattern)]
  stop(
    label,
...
CHILD_COL <- resolve_column(
  CHILD_COL,
  names(raw),
  c("Suivi sanitaire", "plus cas de mortalité", "Djallonké", "métis", "Non"),
  "Djallonke-more-mortality /Non modality",
  "suivisanitaire|pluscasdemortalite|djallonke"
)
...
COMMUNE_COL <- resolve_column(
  ...
  "Commune",
  "commune|caracteristiquesdelunitedelevage"
)
...
data <- raw |>
  ...
  mutate(
    ...
    mortality_no_modality = factor(
      ifelse(child_code %in% c("0", "1"), child_code, NA_character_),
      levels = c("0", "1"),
      labels = c("Not selected", "Non selected")
    )
  )
...
run_test <- function(zone_var, label) {
  d <- data |>
    filter(
      !is.na(mortality_no_modality),
      !is.na(.data[[zone_var]]),
      .data[[zone_var]] != ""
    ) |>
    droplevels()

  observed <- table(d$mortality_no_modality, d[[zone_var]])
  n <- sum(observed)
  nr <- nrow(observed)
  nc <- ncol(observed)
  cells <- length(observed)
  df_chi <- (nr - 1L) * (nc - 1L)

  if (n == 0L || nr < 2L || nc < 2L || cells < 2L) {
    reason <- if (n == 0L) {
      "Not testable: no complete valid observations."
    } else if (nr < 2L) {
      "Not testable: only one response category is observed."
    } else {
      "Not testable: only one zone category is represented."
    }
    ...
    summary_row <- data.frame(
      ...
      Cramers_V = NA_real_, Recommendation = reason,
      stringsAsFactors = FALSE
    )

    percentages <- if (n > 0L && nc > 0L) prop.table(observed, margin = 2) * 100 else NULL
    return(list(summary = summary_row, observed = observed, expected = NULL,
                percent = percentages, stdres = NULL))
  }
  ...
  asymptotic_valid <- n_below_1 == 0L && pct_below_5 <= 20 && margins_ok
  ...
  mc_se <- sqrt(monte_carlo$p.value * (1 - monte_carlo$p.value) / (B + 1))
  mc_low <- max(0, monte_carlo$p.value - 1.96 * mc_se)
  mc_high <- min(1, monte_carlo$p.value + 1.96 * mc_se)

  selected_p <- if (asymptotic_valid) pearson$p.value else monte_carlo$p.value
  selected_test <- if (asymptotic_valid) {
    "Pearson chi-square (asymptotic p-value)"
  } else {
    paste0("Pearson chi-square with Monte Carlo p-value (B=", B, ")")
  }

  denom <- n * min(nr - 1L, nc - 1L)
  cramers_v <- if (denom > 0) sqrt(as.numeric(pearson$statistic) / denom) else NA_real_

  summary_row <- data.frame(
    Comparison = label, N = n, Rows = nr, Columns = nc, Cell_count = length(expected),
    ...
    Asymptotic_chi_square_valid = ifelse(asymptotic_valid, "YES", "NO"),
    ...
    Decision = ifelse(
      selected_p < ALPHA,
      "Reject H0: statistically significant association",
      "Do not reject H0: no statistically significant association"
    ),
    Cramers_V = cramers_v,
...
veg <- run_test(
  "vegetation_zone",
  "Djallonke more mortality cases - Non modality vs vegetation zone"
)
phyto <- run_test(
  "phytogeo_zone",
  "Djallonke more mortality cases - Non modality vs phytogeographic zone"
)
summary_results <- bind_rows(veg$summary, phyto$summary)

quality <- data.frame(
  Metric = c(
    "Total records", "Valid binary codes", "Not selected (0)", "Non selected (1)",
    "Blank responses", "Invalid response codes", "Unmatched communes"
  ),
  Value = c(
    nrow(data),
    sum(!is.na(data$mortality_no_modality)),
...
notes <- data.frame(
  Parameter = c(
    "Variable type", "Coding", "Meaning of analytical Yes", "Null hypothesis",
    "Expected-count rule", "Monte Carlo", "Effect size", "Multiple-modality caution"
  ),
  Assessment = c(
    "Standalone binary qualitative modality.",
    "0=the Non modality was not selected; 1=the Non modality was selected.",
    "The respondent selected Non, meaning more mortality among Djallonke goats was not reported.",
    ...
    "100,000 simulations; selected when the Pearson expected-count conditions fail.",
    "Cramer's V.",
    "If Oui, Non, and other response modalities are mutually exclusive complements, report one combined response variable rather than duplicate modality tests."
  ),
  stringsAsFactors = FALSE
)
