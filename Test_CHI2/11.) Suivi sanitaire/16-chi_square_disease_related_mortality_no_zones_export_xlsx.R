# Disease-related animal mortality, No modality, vs vegetation and phytogeographic zones
# Inputs: data7.xlsx and zones.xlsx
# Output: chi_square_results_disease_related_mortality_no_zones.xlsx
...
OUTPUT_FILE <- "chi_square_results_disease_related_mortality_no_zones.xlsx"
ALPHA <- 0.05
B <- 100000L
SEED <- 20260914L
CHILD_COL <- "11.) Suivi sanitaire/Y-a-t-il des cas de mortalités liées aux maladies /Non"
...
CHILD_COL <- resolve_column(CHILD_COL,names(raw),c("Suivi sanitaire","cas de mortalités","liées aux maladies","Non"),"Disease-related mortality No selected")
...
data <- raw |>
  ...
  mutate(eligible=TRUE,invalid_child=!is.na(child_code)&child_code!=""&!child_code%in%c("0","1"),disease_related_mortality_no=factor(ifelse(child_code%in%c("0","1"),child_code,NA_character_),levels=c("0","1"),labels=c("No","Yes")))
run_test <- function(zone_var,label){
  d <- data |> filter(eligible,!is.na(disease_related_mortality_no),!is.na(.data[[zone_var]]),.data[[zone_var]]!="") |> droplevels()
  obs <- table(d$disease_related_mortality_no,d[[zone_var]]); n<-sum(obs); nr<-nrow(obs); nc<-ncol(obs); dfchi<-(nr-1)*(nc-1)
  if(n==0L||nr<2L||nc<2L){
