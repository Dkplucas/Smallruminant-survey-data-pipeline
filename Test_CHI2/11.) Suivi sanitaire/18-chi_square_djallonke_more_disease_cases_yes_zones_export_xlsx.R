# More disease cases in Djallonke goats than crossbred and other breeds, Yes modality, vs vegetation and phytogeographic zones
# Inputs: data7.xlsx and zones.xlsx
# Output: chi_square_results_djallonke_more_disease_cases_yes_zones.xlsx
...
OUTPUT_FILE <- "chi_square_results_djallonke_more_disease_cases_yes_zones.xlsx"
ALPHA <- 0.05
B <- 100000L
SEED <- 20260914L
CHILD_COL <- "11.) Suivi sanitaire/Enregistrez- vous plus de cas de maladies chez les chèvres de races Djallonké que chez les métis et autres races dans votre élevage ? /Oui"
...
CHILD_COL <- resolve_column(CHILD_COL,names(raw),c("Suivi sanitaire","plus de cas de maladies","chèvres","Djallonké","métis","autres races","Oui"),"More disease cases in Djallonke goats Yes selected")
...
data <- raw |>
  ...
  mutate(eligible=TRUE,invalid_child=!is.na(child_code)&child_code!=""&!child_code%in%c("0","1"),djallonke_more_disease_cases_yes=factor(ifelse(child_code%in%c("0","1"),child_code,NA_character_),levels=c("0","1"),labels=c("No","Yes")))
run_test <- function(zone_var,label){
  d <- data |> filter(eligible,!is.na(djallonke_more_disease_cases_yes),!is.na(.data[[zone_var]]),.data[[zone_var]]!="") |> droplevels()
  obs <- table(d$djallonke_more_disease_cases_yes,d[[zone_var]]); n<-sum(obs); nr<-nrow(obs); nc<-ncol(obs); dfchi<-(nr-1)*(nc-1)
  if(n==0L||nr<2L||nc<2L){
...
veg<-run_test("vegetation_zone","More disease cases in Djallonke goats Yes x vegetation zone")
phyto<-run_test("phytogeo_zone","More disease cases in Djallonke goats Yes x phytogeographic zone")
quality<-data.frame(Metric=c("Total records","Valid eligible child responses","No (0)","Yes (1)","Blank child responses","Invalid child codes","Unmatched communes"),Value=c(nrow(data),sum(data$eligible&!is.na(data$djallonke_more_disease_cases_yes),na.rm=TRUE),sum(data$eligible&data$child_code=="0",na.rm=TRUE),sum(data$eligible&data$child_code=="1",na.rm=TRUE),sum(is.na(data$child_code)|data$child_code==""),sum(data$invalid_child),sum(is.na(data$vegetation_zone))))
notes<-data.frame(Parameter=c("Variable type","Analysis population","Coding","Null hypothesis","Expected-count rule","Monte Carlo","Effect size"),Assessment=c("Standalone binary multiple-response qualitative modality.","All records with a valid 0/1 response and matched zone are eligible.","0=option not selected; 1=more disease cases in Djallonke goats Yes selected selected.","Selection of more disease cases in Djallonke goats Yes selected and geographical zone are independent.","No expected count below 1 and no more than 20% below 5.","100,000 simulations, selected when Pearson assumptions fail.","Cramer's V."),stringsAsFactors=FALSE)
