# Perceived reasons for recurrent disease occurrence vs vegetation and phytogeographic zones
# Inputs: data7.xlsx and zones.xlsx
# Output: chi_square_results_recurrent_disease_reasons_zones.xlsx
...
OUTPUT_FILE <- "chi_square_results_recurrent_disease_reasons_zones.xlsx"
ALPHA <- 0.05
B <- 100000L
SEED <- 20260914L

DISEASE_REASON_COL <-
  "11.) Suivi sanitaire/Selon vous, pourquoi ? 0=Vides, 1=Sanitaire, 2=Climat, 3=Forrage"
PARENT_DISEASE_TIMING_COL <-
  "11.) Suivi sanitaire/A quel moment apparaissent-elles ? 0=Vide, 1=Saison Seche, 2=Saison Pluvieuse, 3=Autres"
...
DISEASE_REASON_COL <- resolve_column(
  DISEASE_REASON_COL,
  names(raw),
  c("Suivi sanitaire", "Selon vous", "pourquoi", "Sanitaire", "Climat", "Forrage"),
  "Perceived disease-occurrence reason"
)

PARENT_DISEASE_TIMING_COL <- resolve_column(
  PARENT_DISEASE_TIMING_COL,
  names(raw),
  c("Suivi sanitaire", "A quel moment", "Saison Seche", "Saison Pluvieuse", "Autres"),
  "Parent recurrent-disease timing"
)
...
data <- raw |>
  transmute(
    ...
    disease_reason_code = str_squish(.data[[DISEASE_REASON_COL]]),
    parent_timing_code = str_squish(.data[[PARENT_DISEASE_TIMING_COL]])
  ) |>
  left_join(zones, by = "commune_key") |>
  mutate(
    invalid_reason_code = !is.na(disease_reason_code) &
      disease_reason_code != "" &
      !disease_reason_code %in% c("0", "1", "2", "3"),
    invalid_parent_code = !is.na(parent_timing_code) &
      parent_timing_code != "" &
      !parent_timing_code %in% c("0", "1", "2", "3"),
    eligible = parent_timing_code %in% c("1", "2", "3"),
    ineligible_substantive_reason = !eligible &
      disease_reason_code %in% c("1", "2", "3"),
    disease_reason = factor(
      ifelse(
        eligible & disease_reason_code %in% c("1", "2", "3"),
        disease_reason_code,
        NA_character_
      ),
      levels = c("1", "2", "3"),
      labels = c("Health-related", "Climate", "Fodder"),
      ordered = FALSE
    )
  )
...
run_test <- function(zone_var, label) {
  d <- data |>
    filter(
      !is.na(disease_reason),
      !is.na(.data[[zone_var]]),
      .data[[zone_var]] != ""
    ) |>
    droplevels()

  observed <- table(d$disease_reason, d[[zone_var]])
  n <- sum(observed)
  nr <- nrow(observed)
  nc <- ncol(observed)
  df_chi <- (nr - 1) * (nc - 1)
...
veg <- run_test(
  "vegetation_zone",
  "Perceived disease-occurrence reason x vegetation zone"
)
phyto <- run_test(
  "phytogeo_zone",
  "Perceived disease-occurrence reason x phytogeographic zone"
)
...
quality <- data.frame(
  Metric = c(
    "Total records",
    "Eligible parent timing responses (1/2/3)",
    "Valid substantive reason responses",
    "Empty/not applicable reason code (0)",
    "Health-related (1)",
    "Climate (2)",
    "Fodder (3)",
    "Blank reason responses",
    "Invalid reason codes",
    "Invalid parent timing codes",
    "Substantive reason among ineligible records",
    "Unmatched communes"
  ),
  Value = c(
    nrow(data),
    sum(data$eligible, na.rm = TRUE),
    sum(!is.na(data$disease_reason)),
    sum(data$disease_reason_code == "0", na.rm = TRUE),
    sum(data$disease_reason_code == "1", na.rm = TRUE),
    sum(data$disease_reason_code == "2", na.rm = TRUE),
    sum(data$disease_reason_code == "3", na.rm = TRUE),
    sum(is.na(data$disease_reason_code) | data$disease_reason_code == ""),
    sum(data$invalid_reason_code, na.rm = TRUE),
    sum(data$invalid_parent_code, na.rm = TRUE),
    sum(data$ineligible_substantive_reason, na.rm = TRUE),
...
notes <- data.frame(
  ...
  Assessment = c(
    "Conditional nominal qualitative variable with three substantive categories.",
    "Eligible when parent disease-timing code is 1=Dry season, 2=Rainy season, or 3=Other timing.",
    "1=Health-related; 2=Climate; 3=Fodder.",
    "Code 0 means empty/not applicable and is excluded from association tests.",
    "Perceived disease-occurrence reason and geographical zone are independent among eligible records.",
