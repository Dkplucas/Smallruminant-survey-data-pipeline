# Entire-herd deworming option vs vegetation and phytogeographic zones
# Inputs: data7.xlsx and zones.xlsx
# Output: chi_square_results_dewormed_animals_entire_herd_zones.xlsx
...
OUTPUT_FILE <- "chi_square_results_dewormed_animals_entire_herd_zones.xlsx"
ALPHA <- 0.05
B <- 100000L
SEED <- 20260914L

DEWORMED_ENTIRE_HERD_COL <-
  "11.) Suivi sanitaire/Quels sont les animaux déparasités ? /1= Tout le troupeau"
...
DEWORMED_ENTIRE_HERD_COL <- resolve_column(
  DEWORMED_ENTIRE_HERD_COL,
  names(raw),
  c("Suivi sanitaire", "animaux déparasités", "Tout le troupeau"),
  "Entire-herd deworming option"
)
...
data <- raw |>
  transmute(
    ...
    entire_herd_code = str_squish(.data[[DEWORMED_ENTIRE_HERD_COL]]),
    parent_deworming_yes_code = str_squish(.data[[PARENT_DEWORMING_YES_COL]])
  ) |>
  left_join(zones, by = "commune_key") |>
  mutate(
    invalid_child_code = !is.na(entire_herd_code) & entire_herd_code != "" & !entire_herd_code %in% c("0", "1"),
    ...
    ineligible_positive = !eligible & entire_herd_code == "1",
    entire_herd = factor(
      ifelse(eligible & entire_herd_code %in% c("0", "1"), entire_herd_code, NA_character_),
      levels = c("0", "1"),
      labels = c("No", "Yes")
    )
  )
...
run_test <- function(zone_var, label) {
  ...
  observed <- table(d$entire_herd, d[[zone_var]])
  n <- sum(observed)
  nr <- nrow(observed)
  nc <- ncol(observed)
  df_chi <- (nr - 1) * (nc - 1)
...
veg <- run_test(
  "vegetation_zone",
  "Entire herd selected for deworming x vegetation zone"
)
phyto <- run_test(
  "phytogeo_zone",
  "Entire herd selected for deworming x phytogeographic zone"
)

summary_results <- bind_rows(veg$summary, phyto$summary)

quality <- data.frame(
  Metric = c("Total records", "Eligible respondents selecting deworming Yes", "Valid eligible binary responses", "Entire herd not selected (0)", "Entire herd selected (1)", "Blank child responses", "Invalid child codes", "Invalid parent codes", "Ineligible positive responses", "Unmatched communes"),
  Value = c(nrow(data), sum(data$eligible, na.rm = TRUE), sum(!is.na(data$entire_herd)), sum(data$eligible & data$entire_herd_code == "0", na.rm = TRUE), sum(data$eligible & data$entire_herd_code == "1", na.rm = TRUE), sum(is.na(data$entire_herd_code) | data$entire_herd_code == ""), sum(data$invalid_child_code, na.rm = TRUE), sum(data$invalid_parent_code, na.rm = TRUE), sum(data$ineligible_positive, na.rm = TRUE), sum(is.na(data$vegetation_zone))),
  stringsAsFactors = FALSE
)

notes <- data.frame(
  Parameter = c("Variable type", "Parent eligibility", "Coding", "Null hypothesis", "Expected-count rule", "Monte Carlo", "Effect size"),
  Assessment = c("Conditional binary qualitative multiple-response option.", "Eligible only when the parent deworming Yes modality equals 1.", "0=Entire herd not selected; 1=Entire herd selected.", "Selection of the entire-herd deworming option and geographical zone are independent among eligible respondents.", "No expected count below 1 and no more than 20% below 5.", "100,000 simulations, selected when Pearson expected-count conditions fail.", "Cramer's V."),
  stringsAsFactors = FALSE
)
