# Deworming of animals, Yes modality, vs vegetation and phytogeographic zones
# Inputs: data7.xlsx and zones.xlsx
# Output: chi_square_results_deworming_yes_zones.xlsx
...
OUTPUT_FILE <- "chi_square_results_deworming_yes_zones.xlsx"
ALPHA <- 0.05
B <- 100000L
SEED <- 20260914L
CHILD_COL <- "11.) Suivi sanitaire/Déparasitez-vous vos animaux ? /Oui"
...
CHILD_COL <- resolve_column(CHILD_COL,names(raw),c("Suivi sanitaire","Déparasitez-vous vos animaux","Oui"),"Deworming selected")
...
data <- raw |>
  ...
  mutate(eligible=TRUE,invalid_child=!is.na(child_code)&child_code!=""&!child_code%in%c("0","1"),deworming_yes=factor(ifelse(child_code%in%c("0","1"),child_code,NA_character_),levels=c("0","1"),labels=c("No","Yes")))
run_test <- function(zone_var,label){
  d <- data |> filter(eligible,!is.na(deworming_yes),!is.na(.data[[zone_var]]),.data[[zone_var]]!="") |> droplevels()
  obs <- table(d$deworming_yes,d[[zone_var]]); n<-sum(obs); nr<-nrow(obs); nc<-ncol(obs); dfchi<-(nr-1)*(nc-1)
  if(n==0L||nr<2L||nc<2L){
