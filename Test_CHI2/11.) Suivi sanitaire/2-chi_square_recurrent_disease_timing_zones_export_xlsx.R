# Timing of recurrent diseases vs vegetation and phytogeographic zones
# Inputs: data7.xlsx and zones.xlsx
# Output: chi_square_results_recurrent_disease_timing_zones.xlsx
...
OUTPUT_FILE <- "chi_square_results_recurrent_disease_timing_zones.xlsx"
ALPHA <- 0.05
B <- 100000L
SEED <- 20260914L
...
data <- raw |>
  transmute(
    ...
    disease_timing_code = str_squish(.data[[DISEASE_TIMING_COL]]),
    parent_disease_code = str_squish(.data[[PARENT_RECURRENT_DISEASES_COL]])
  ) |>
  left_join(zones, by = "commune_key") |>
  mutate(
    invalid_timing_code = !is.na(disease_timing_code) &
      disease_timing_code != "" &
      !disease_timing_code %in% c("0", "1", "2", "3"),
    invalid_parent_code = !is.na(parent_disease_code) &
      parent_disease_code != "" &
      !parent_disease_code %in% c("0", "1", "2"),
    eligible = parent_disease_code %in% c("1", "2"),
    ineligible_substantive_timing = !eligible &
      disease_timing_code %in% c("1", "2", "3"),
    disease_timing = factor(
      ifelse(
        eligible & disease_timing_code %in% c("1", "2", "3"),
        disease_timing_code,
        NA_character_
      ),
      levels = c("1", "2", "3"),
      labels = c("Dry season", "Rainy season", "Other timing"),
      ordered = FALSE
    )
  )
...
run_test <- function(zone_var, label) {
  d <- data |>
    filter(
      !is.na(disease_timing),
      !is.na(.data[[zone_var]]),
      .data[[zone_var]] != ""
    ) |>
    droplevels()

  observed <- table(d$disease_timing, d[[zone_var]])
  n <- sum(observed)
  nr <- nrow(observed)
  nc <- ncol(observed)
  df_chi <- (nr - 1) * (nc - 1)
...
veg <- run_test(
  "vegetation_zone",
  "Recurrent-disease timing x vegetation zone"
)
phyto <- run_test(
  "phytogeo_zone",
  "Recurrent-disease timing x phytogeographic zone"
)
...
quality <- data.frame(
  Metric = c(
    "Total records",
    "Eligible parent responses (1 or 2)",
    "Valid substantive timing responses",
    "Empty/not applicable timing code (0)",
    "Dry season (1)",
    "Rainy season (2)",
    "Other timing (3)",
    "Blank timing responses",
    "Invalid timing codes",
    "Invalid parent codes",
    "Substantive timing among ineligible records",
    "Unmatched communes"
  ),
  Value = c(
    nrow(data),
    sum(data$eligible, na.rm = TRUE),
    sum(!is.na(data$disease_timing)),
    sum(data$disease_timing_code == "0", na.rm = TRUE),
    sum(data$disease_timing_code == "1", na.rm = TRUE),
    sum(data$disease_timing_code == "2", na.rm = TRUE),
    sum(data$disease_timing_code == "3", na.rm = TRUE),
    sum(is.na(data$disease_timing_code) | data$disease_timing_code == ""),
    sum(data$invalid_timing_code, na.rm = TRUE),
...
notes <- data.frame(
  ...
  Assessment = c(
    ...
    "Eligible when recurrent-disease parent code is 1=Diarrhea or 2=Other diseases.",
    "1=Dry season; 2=Rainy season; 3=Other timing.",
    "Code 0 means empty/not applicable and is excluded from association tests.",
    "Disease timing and geographical zone are independent among eligible records.",
