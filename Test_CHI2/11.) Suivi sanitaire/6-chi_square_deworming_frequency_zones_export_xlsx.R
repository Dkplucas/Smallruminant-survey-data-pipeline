# Annual deworming frequency vs vegetation and phytogeographic zones
# Inputs: data7.xlsx and zones.xlsx
# Output: chi_square_results_deworming_frequency_zones.xlsx
...
OUTPUT_FILE <- "chi_square_results_deworming_frequency_zones.xlsx"
ALPHA <- 0.05
B <- 100000L
SEED <- 20260914L

DEWORMING_FREQUENCY_COL <-
  "11.) Suivi sanitaire/Si oui, avec quelle fréquence par ans? 0=Vides, 1=[1:3], 2=[4:6], 3=Autres"
...
DEWORMING_FREQUENCY_COL <- resolve_column(
  DEWORMING_FREQUENCY_COL,
  names(raw),
  c("Suivi sanitaire", "Si oui", "fréquence par ans", "1:3", "4:6", "Autres"),
  "Annual deworming frequency"
)
...
data <- raw |>
  transmute(
    ...
    deworming_frequency_code = str_squish(.data[[DEWORMING_FREQUENCY_COL]]),
    parent_deworming_yes_code = str_squish(.data[[PARENT_DEWORMING_YES_COL]])
  ) |>
  left_join(zones, by = "commune_key") |>
  mutate(
    invalid_frequency_code = !is.na(deworming_frequency_code) &
      deworming_frequency_code != "" &
      !deworming_frequency_code %in% c("0", "1", "2", "3"),
    invalid_parent_code = !is.na(parent_deworming_yes_code) &
      parent_deworming_yes_code != "" &
      !parent_deworming_yes_code %in% c("0", "1"),
    eligible = parent_deworming_yes_code == "1",
    ineligible_substantive_frequency = !eligible &
      deworming_frequency_code %in% c("1", "2", "3"),
    deworming_frequency = factor(
      ifelse(
        eligible & deworming_frequency_code %in% c("1", "2", "3"),
        deworming_frequency_code,
        NA_character_
      ),
      levels = c("1", "2", "3"),
      labels = c("1-3 times per year", "4-6 times per year", "Other frequency"),
      ordered = TRUE
    )
  )
...
run_test <- function(zone_var, label) {
  d <- data |>
    filter(
      !is.na(deworming_frequency),
      !is.na(.data[[zone_var]]),
      .data[[zone_var]] != ""
    ) |>
    droplevels()

  observed <- table(d$deworming_frequency, d[[zone_var]])
  n <- sum(observed)
  nr <- nrow(observed)
  nc <- ncol(observed)
  df_chi <- (nr - 1) * (nc - 1)
...
veg <- run_test(
  "vegetation_zone",
  "Annual deworming frequency x vegetation zone"
)
phyto <- run_test(
  "phytogeo_zone",
  "Annual deworming frequency x phytogeographic zone"
)
...
quality <- data.frame(
  Metric = c(
    "Total records",
    "Eligible respondents selecting deworming Yes",
    "Valid substantive frequency responses",
    "Empty/not applicable frequency code (0)",
    "1-3 times per year (1)",
    "4-6 times per year (2)",
    "Other frequency (3)",
    "Blank frequency responses",
    "Invalid frequency codes",
    "Invalid parent codes",
    "Substantive frequency among ineligible records",
    "Unmatched communes"
  ),
  Value = c(
    nrow(data),
    sum(data$eligible, na.rm = TRUE),
    sum(!is.na(data$deworming_frequency)),
    sum(data$deworming_frequency_code == "0", na.rm = TRUE),
    sum(data$deworming_frequency_code == "1", na.rm = TRUE),
    sum(data$deworming_frequency_code == "2", na.rm = TRUE),
    sum(data$deworming_frequency_code == "3", na.rm = TRUE),
    sum(is.na(data$deworming_frequency_code) | data$deworming_frequency_code == ""),
    sum(data$invalid_frequency_code, na.rm = TRUE),
    sum(data$invalid_parent_code, na.rm = TRUE),
    sum(data$ineligible_substantive_frequency, na.rm = TRUE),
...
notes <- data.frame(
  ...
  Assessment = c(
    "Conditional ordinal qualitative variable with three substantive categories.",
    "Eligible only when the parent deworming Yes modality equals 1.",
    "1=1-3 times/year; 2=4-6 times/year; 3=Other frequency.",
    "Code 0 means empty/not applicable and is excluded from association tests.",
    "Annual deworming frequency and geographical zone are independent among eligible records.",
    ...
    "Cramer's V. The chi-square test treats categories nominally despite their ordered display."
  ),
  stringsAsFactors = FALSE
)
