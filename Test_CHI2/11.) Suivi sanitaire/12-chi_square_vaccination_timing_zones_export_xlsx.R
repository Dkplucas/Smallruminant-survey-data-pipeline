# Vaccination timing/frequency vs vegetation and phytogeographic zones
# Inputs: data7.xlsx and zones.xlsx
# Output: chi_square_results_vaccination_timing_zones.xlsx
...
OUTPUT_FILE <- "chi_square_results_vaccination_timing_zones.xlsx"
ALPHA <- 0.05
B <- 100000L
SEED <- 20260914L

VACCINATION_TIMING_COL <-
  "11.) Suivi sanitaire/Si oui, à quel moment de l’année ? 0=Vides, 1={1,2 par ans}, 2=Autres"
PARENT_VACCINATION_YES_COL <-
  "11.) Suivi sanitaire/Vaccinez-vous vos animaux ? /Oui"
...
VACCINATION_TIMING_COL <- resolve_column(
  VACCINATION_TIMING_COL,
  names(raw),
  c("Suivi sanitaire", "Si oui", "moment de l’année", "1,2 par ans", "Autres"),
  "Vaccination timing/frequency"
)

PARENT_VACCINATION_YES_COL <- resolve_column(
  PARENT_VACCINATION_YES_COL,
  names(raw),
  c("Suivi sanitaire", "Vaccinez-vous vos animaux", "Oui"),
  "Parent vaccination Yes modality"
)
...
data <- raw |>
  transmute(
    ...
    vaccination_timing_code = str_squish(.data[[VACCINATION_TIMING_COL]]),
    parent_vaccination_yes_code = str_squish(.data[[PARENT_VACCINATION_YES_COL]])
  ) |>
  left_join(zones, by = "commune_key") |>
  mutate(
    invalid_frequency_code = !is.na(vaccination_timing_code) &
      vaccination_timing_code != "" &
      !vaccination_timing_code %in% c("0", "1", "2"),
    invalid_parent_code = !is.na(parent_vaccination_yes_code) &
      parent_vaccination_yes_code != "" &
      !parent_vaccination_yes_code %in% c("0", "1"),
    eligible = parent_vaccination_yes_code == "1",
    ineligible_substantive_timing = !eligible &
      vaccination_timing_code %in% c("1", "2"),
    vaccination_timing = factor(
      ifelse(
        eligible & vaccination_timing_code %in% c("1", "2"),
        vaccination_timing_code,
        NA_character_
      ),
      levels = c("1", "2"),
      labels = c("1-2 times per year", "Other timing/frequency"),
      ordered = FALSE
    )
  )
...
run_test <- function(zone_var, label) {
  d <- data |>
    filter(
      !is.na(vaccination_timing),
      !is.na(.data[[zone_var]]),
      .data[[zone_var]] != ""
    ) |>
    droplevels()

  observed <- table(d$vaccination_timing, d[[zone_var]])
  n <- sum(observed)
  nr <- nrow(observed)
  nc <- ncol(observed)
  df_chi <- (nr - 1) * (nc - 1)
...
veg <- run_test(
  "vegetation_zone",
  "Vaccination timing/frequency x vegetation zone"
)
phyto <- run_test(
  "phytogeo_zone",
  "Vaccination timing/frequency x phytogeographic zone"
)
...
quality <- data.frame(
  Metric = c(
    "Total records",
    "Eligible: parent vaccination Yes selected",
    "Valid eligible substantive responses",
    "1-2 times per year (1)",
    "Other timing/frequency (2)",
    "Empty/not applicable code (0)",
    "Blank child responses",
    "Invalid child codes",
    "Invalid parent codes",
    "Ineligible substantive responses",
    "Unmatched communes"
  ),
  Value = c(
    nrow(data),
    sum(data$eligible, na.rm = TRUE),
    sum(!is.na(data$vaccination_timing)),
    sum(data$eligible & data$vaccination_timing_code == "1", na.rm = TRUE),
    sum(data$eligible & data$vaccination_timing_code == "2", na.rm = TRUE),
    sum(data$vaccination_timing_code == "0", na.rm = TRUE),
    sum(is.na(data$vaccination_timing_code) | data$vaccination_timing_code == ""),
    sum(data$invalid_child_code, na.rm = TRUE),
    sum(data$invalid_parent_code, na.rm = TRUE),
    sum(data$ineligible_substantive_timing, na.rm = TRUE),
...
notes <- data.frame(
  ...
  Assessment = c(
    "Conditional nominal qualitative variable.",
    "Eligible only when the parent vaccination /Oui modality equals 1.",
    "0=Empty/not applicable; 1=1-2 times per year; 2=Other timing/frequency.",
    "Code 0 is excluded from substantive association tests.",
    "Vaccination timing/frequency and geographical zone are independent among eligible respondents.",
