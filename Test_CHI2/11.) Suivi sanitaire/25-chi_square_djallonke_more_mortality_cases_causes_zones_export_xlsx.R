...
# Output: chi_square_results_djallonke_more_mortality_cases_causes_zones.xlsx
...
OUTPUT_FILE <- "chi_square_results_djallonke_more_mortality_cases_causes_zones.xlsx"
ALPHA <- 0.05
B <- 100000L
SEED <- 20260914L

DJALLONKE_MORE_MORTALITY_CASES_CAUSES_COL <-
  "11.) Suivi sanitaire/Si oui, quelles sont les causes selon vous ?...246/ 0=Vides, 1=Du au climat, 2=Faute au traitement, 3=Race metis fragile"
PARENT_DJALLONKE_MORE_MORTALITY_CASES_YES_COL <-
  "11.) Suivi sanitaire/Enregistrez-vous plus cas de mortalité chez les chèvres de races Djallonké que chez les métis et autres races dans votre élevage ? /Oui"
...
DJALLONKE_MORE_MORTALITY_CASES_CAUSES_COL <- resolve_column(
  DJALLONKE_MORE_MORTALITY_CASES_CAUSES_COL,
  names(raw),
...
PARENT_DJALLONKE_MORE_MORTALITY_CASES_YES_COL <- resolve_column(
  PARENT_DJALLONKE_MORE_MORTALITY_CASES_YES_COL,
  names(raw),
...
data <- raw |>
  transmute(
    ...
    disease_cause_code = str_squish(.data[[DJALLONKE_MORE_MORTALITY_CASES_CAUSES_COL]]),
    parent_more_mortality_cases_yes_code = str_squish(.data[[PARENT_DJALLONKE_MORE_MORTALITY_CASES_YES_COL]])
  ) |>
  left_join(zones, by = "commune_key") |>
  mutate(
    invalid_cause_code = !is.na(disease_cause_code) &
      disease_cause_code != "" &
      !disease_cause_code %in% c("0", "1", "2", "3"),
    invalid_parent_code = !is.na(parent_more_mortality_cases_yes_code) &
      parent_more_mortality_cases_yes_code != "" &
      !parent_more_mortality_cases_yes_code %in% c("0", "1"),
    eligible = parent_more_mortality_cases_yes_code == "1",
    ineligible_substantive_cause = !eligible &
      disease_cause_code %in% c("1", "2", "3"),
    disease_cause = factor(
      ifelse(
        eligible & disease_cause_code %in% c("1", "2", "3"),
        disease_cause_code,
        NA_character_
      ),
      levels = c("1", "2", "3"),
      labels = c("Due to climate", "Treatment failure", "Fragile crossbred breed"),
      ordered = FALSE
    )
  )
...
quality <- data.frame(
  ...
  Value = c(
    ...
    sum(data$eligible & data$disease_cause_code == "3", na.rm = TRUE),
...
notes <- data.frame(
  ...
  Assessment = c(
    ...
    "Eligible only when the parent /Oui modality for more mortality cases in Djallonke goats equals 1.",
    "0=Empty/not applicable; 1=Due to climate; 2=Treatment failure.",
