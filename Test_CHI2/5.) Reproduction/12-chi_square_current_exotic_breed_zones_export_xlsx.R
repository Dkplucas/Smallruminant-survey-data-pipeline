# Current exotic breed in herd (No/Yes) vs vegetation and phytogeographic zones
# Inputs: data7.xlsx and zones.xlsx
# Output: chi_square_results_current_exotic_breed_zones.xlsx
...
DATA_FILE<-"data7.xlsx";ZONES_FILE<-"zones.xlsx";OUTPUT_FILE<-"chi_square_results_current_exotic_breed_zones.xlsx"
ALPHA<-0.05;B<-100000L;SEED<-20260828L
CURRENT_EXOTIC_BREED_COL<-"5.) Reproduction/Avez-vous actuellement une race exotique dans votre troupeau ? 0=Non, 1=Oui"
...
if(!CURRENT_EXOTIC_BREED_COL%in%names(raw))stop("Current-exotic-breed column not found exactly in data7.xlsx");if(!COMMUNE_COL%in%names(raw))stop("Commune column not found exactly")
...
data<-raw|>transmute(current_exotic_breed_code=str_squish(.data[[CURRENT_EXOTIC_BREED_COL]]),commune_original=str_squish(.data[[COMMUNE_COL]]),commune_key=normalize_key(.data[[COMMUNE_COL]]))|>mutate(current_exotic_breed=factor(current_exotic_breed_code,levels=c("0","1"),labels=c("No","Yes")),invalid_current_exotic_breed=is.na(current_exotic_breed)&!is.na(current_exotic_breed_code)&current_exotic_breed_code!="")|>left_join(zones,by="commune_key")
run_test<-function(df,zone_var,label){
 d<-df|>filter(!is.na(current_exotic_breed),!is.na(.data[[zone_var]]))|>droplevels();obs<-table(d$current_exotic_breed,d[[zone_var]]);pearson<-suppressWarnings(chisq.test(obs,correct=FALSE));exp<-pearson$expected;n<-sum(obs);cells<-length(exp);dfchi<-(nrow(obs)-1)*(ncol(obs)-1);n5<-sum(exp<5);n1<-sum(exp<1);pct5<-100*n5/cells;margins<-!any(rowSums(obs)==0)&!any(colSums(obs)==0);valid<-n1==0&pct5<=20&margins
...
veg<-run_test(data,"vegetation_zone","Current exotic breed in herd vs vegetation zone");phyto<-run_test(data,"phytogeo_zone","Current exotic breed in herd vs phytogeographic zone");summary<-bind_rows(veg$summary,phyto$summary)
matrix_df<-function(x){
  ...
  out<-data.frame(`Current exotic breed in herd`=rn,stringsAsFactors=FALSE,check.names=FALSE)
  for(j in seq_len(nc))out[[cn[j]]]<-unname(m[,j])
  out
}
quality<-data.frame(Parameter=c("Rows read","Valid current-exotic-breed records","No","Yes","Missing responses","Invalid codes","Unmatched zone records"),Value=c(nrow(data),sum(!is.na(data$current_exotic_breed)),sum(data$current_exotic_breed=="No",na.rm=TRUE),sum(data$current_exotic_breed=="Yes",na.rm=TRUE),sum(is.na(data$current_exotic_breed_code)|data$current_exotic_breed_code==""),sum(data$invalid_current_exotic_breed),sum(is.na(data$vegetation_zone))))
notes<-data.frame(Parameter=c("Variable type","Code 0 interpretation","Null hypothesis","Expected-count rule","Monte Carlo","Multiple-response caution","Independence","Survey design"),Assessment=c("Current exotic breed in herd and zone are qualitative variables, so chi-square is appropriate.","The source label defines 0=No and 1=Yes.","Current exotic breed in herd status and zone are independent.","No expected count below 1 and no more than 20% below 5.","Monte Carlo with 100,000 simulations is exported as sensitivity analysis and selected only if expected-count conditions fail.","This is a standalone binary qualitative variable indicating whether an exotic breed is currently present in the herd. All valid livestock-unit responses are included in the analysis.","Each livestock unit contributes once to the current-exotic-breed table.","Use Rao-Scott chi-square if weights, strata, or village clusters apply."))
sheets<-list()
