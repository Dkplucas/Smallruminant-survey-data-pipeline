# Other specified starting goat breed (No/Yes) vs vegetation and phytogeographic zones
# Inputs: data7.xlsx and zones.xlsx
# Output: chi_square_results_starting_goat_breed_other_zones.xlsx
...
DATA_FILE<-"data7.xlsx";ZONES_FILE<-"zones.xlsx";OUTPUT_FILE<-"chi_square_results_starting_goat_breed_other_zones.xlsx"
ALPHA<-0.05;B<-100000L;SEED<-20260828L
STARTING_GOAT_BREED_OTHER_COL<-"5.) Reproduction/Avec quelles races de chèvre avez-vous commencé votre élevage de caprins ? /Autre à préciser/0=Non, 1=Oui"
...
if(!STARTING_GOAT_BREED_OTHER_COL%in%names(raw))stop("Other starting-goat-breed column not found exactly in data7.xlsx");if(!COMMUNE_COL%in%names(raw))stop("Commune column not found exactly")
...
data<-raw|>transmute(starting_goat_breed_other_code=str_squish(.data[[STARTING_GOAT_BREED_OTHER_COL]]),commune_original=str_squish(.data[[COMMUNE_COL]]),commune_key=normalize_key(.data[[COMMUNE_COL]]))|>mutate(starting_goat_breed_other=factor(starting_goat_breed_other_code,levels=c("0","1"),labels=c("No","Yes")),invalid_starting_goat_breed_other=is.na(starting_goat_breed_other)&!is.na(starting_goat_breed_other_code)&starting_goat_breed_other_code!="")|>left_join(zones,by="commune_key")
run_test<-function(df,zone_var,label){
 d<-df|>filter(!is.na(starting_goat_breed_other),!is.na(.data[[zone_var]]))|>droplevels();obs<-table(d$starting_goat_breed_other,d[[zone_var]]);pearson<-suppressWarnings(chisq.test(obs,correct=FALSE));exp<-pearson$expected;n<-sum(obs);cells<-length(exp);dfchi<-(nrow(obs)-1)*(ncol(obs)-1);n5<-sum(exp<5);n1<-sum(exp<1);pct5<-100*n5/cells;margins<-!any(rowSums(obs)==0)&!any(colSums(obs)==0);valid<-n1==0&pct5<=20&margins
...
veg<-run_test(data,"vegetation_zone","Other specified starting goat breed vs vegetation zone");phyto<-run_test(data,"phytogeo_zone","Other specified starting goat breed vs phytogeographic zone");summary<-bind_rows(veg$summary,phyto$summary)
matrix_df<-function(x){
  ...
  out<-data.frame(`Other specified starting goat breed`=rn,stringsAsFactors=FALSE,check.names=FALSE)
  for(j in seq_len(nc))out[[cn[j]]]<-unname(m[,j])
  out
}
quality<-data.frame(Parameter=c("Rows read","Valid starting-goat-breed-sahelian records","No","Yes","Missing responses","Invalid codes","Unmatched zone records"),Value=c(nrow(data),sum(!is.na(data$starting_goat_breed_other)),sum(data$starting_goat_breed_other=="No",na.rm=TRUE),sum(data$starting_goat_breed_other=="Yes",na.rm=TRUE),sum(is.na(data$starting_goat_breed_other_code)|data$starting_goat_breed_other_code==""),sum(data$invalid_starting_goat_breed_other),sum(is.na(data$vegetation_zone))))
notes<-data.frame(Parameter=c("Variable type","Code 0 interpretation","Null hypothesis","Expected-count rule","Monte Carlo","Multiple-response caution","Independence","Survey design"),Assessment=c("Other specified starting goat breed and zone are qualitative variables, so chi-square is appropriate.","The source label defines 0=No and 1=Yes.","Other specified starting goat breed status and zone are independent.","No expected count below 1 and no more than 20% below 5.","Monte Carlo with 100,000 simulations is exported as sensitivity analysis and selected only if expected-count conditions fail.","This is one binary modality of a multiple-response question on the goat breeds used to start the herd. All valid livestock-unit responses are included. Analyze each starting-breed modality separately and adjust p-values when reporting several breed tests.","Each livestock unit contributes once to this other starting-breed table.","Use Rao-Scott chi-square if weights, strata, or village clusters apply."))
sheets<-list()
