# Sahelian breed type present in herd (No/Yes) vs vegetation and phytogeographic zones
# Inputs: data7.xlsx and zones.xlsx
# Output: chi_square_results_sahelian_breed_type_zones.xlsx
...
DATA_FILE<-"data7.xlsx";ZONES_FILE<-"zones.xlsx";OUTPUT_FILE<-"chi_square_results_sahelian_breed_type_present_zones.xlsx"
ALPHA<-0.05;B<-100000L;SEED<-20260828L
SAHELIAN_BREED_TYPE_COL<-"5.) Reproduction/Quelles sont les races sahéliennes présentes dans votre troupeau ? 0=Non, 1=Djabadjaba(Ovin), 2=Tominnougbo(Caprin), 3=Ayogbo(Caprin), 4=HALAHALA"
...
if(!SAHELIAN_BREED_TYPE_COL%in%names(raw))stop("Market-gardening-by-products column not found exactly in data7.xlsx");if(!COMMUNE_COL%in%names(raw))stop("Commune column not found exactly")
...
data<-raw|>transmute(sahelian_breed_type_code=str_squish(.data[[SAHELIAN_BREED_TYPE_COL]]),commune_original=str_squish(.data[[COMMUNE_COL]]),commune_key=normalize_key(.data[[COMMUNE_COL]]))|>mutate(sahelian_breed_type=factor(sahelian_breed_type_code,levels=c("0","1","2","3","4"),labels=c("None","Djabadjaba (Ovin)","Tominnougbo (Caprin)","Ayogbo (Caprin)","HALAHALA")),invalid_sahelian_breed_type=is.na(sahelian_breed_type)&!is.na(sahelian_breed_type_code)&sahelian_breed_type_code!="")|>left_join(zones,by="commune_key")
run_test<-function(df,zone_var,label){
 d<-df|>filter(!is.na(sahelian_breed_type),!is.na(.data[[zone_var]]))|>droplevels();obs<-table(d$sahelian_breed_type,d[[zone_var]]);pearson<-suppressWarnings(chisq.test(obs,correct=FALSE));exp<-pearson$expected;n<-sum(obs);cells<-length(exp);dfchi<-(nrow(obs)-1)*(ncol(obs)-1);n5<-sum(exp<5);n1<-sum(exp<1);pct5<-100*n5/cells;margins<-!any(rowSums(obs)==0)&!any(colSums(obs)==0);valid<-n1==0&pct5<=20&margins
...
veg<-run_test(data,"vegetation_zone","Sahelian breed type present in herd vs vegetation zone");phyto<-run_test(data,"phytogeo_zone","Sahelian breed type present in herd vs phytogeographic zone");summary<-bind_rows(veg$summary,phyto$summary)
matrix_df<-function(x){
  ...
  out<-data.frame(`Sahelian breed type present in herd`=rn,stringsAsFactors=FALSE,check.names=FALSE)
  for(j in seq_len(nc))out[[cn[j]]]<-unname(m[,j])
  out
}
quality<-data.frame(
  Parameter=c("Rows read","Valid Sahelian-breed-type records","None (code 0)","Djabadjaba (code 1)","Tominnougbo (code 2)","Ayogbo (code 3)","HALAHALA (code 4)","Missing responses","Invalid codes","Unmatched zone records"),
  Value=c(nrow(data),sum(!is.na(data$sahelian_breed_type)),sum(data$sahelian_breed_type_code=="0",na.rm=TRUE),sum(data$sahelian_breed_type_code=="1",na.rm=TRUE),sum(data$sahelian_breed_type_code=="2",na.rm=TRUE),sum(data$sahelian_breed_type_code=="3",na.rm=TRUE),sum(data$sahelian_breed_type_code=="4",na.rm=TRUE),sum(is.na(data$sahelian_breed_type_code)|data$sahelian_breed_type_code==""),sum(data$invalid_sahelian_breed_type),sum(is.na(data$vegetation_zone)))
)
notes<-data.frame(Parameter=c("Variable type","Code 0 interpretation","Null hypothesis","Expected-count rule","Monte Carlo","Multiple-response caution","Independence","Survey design"),Assessment=c("Sahelian breed type present in herd and zone are qualitative variables, so chi-square is appropriate.","The source label defines 0=None, 1=Djabadjaba (Ovin), 2=Tominnougbo (Caprin), 3=Ayogbo (Caprin), and 4=HALAHALA.","Sahelian breed type present in herd status and zone are independent.","No expected count below 1 and no more than 20% below 5.","Monte Carlo with 100,000 simulations is exported as sensitivity analysis and selected only if expected-count conditions fail.","This is a standalone nominal qualitative variable with five coded categories. All valid livestock-unit responses are included in the analysis.","Each livestock unit contributes once to the mineral-supplementation table.","Use Rao-Scott chi-square if weights, strata, or village clusters apply."))
sheets<-list()
