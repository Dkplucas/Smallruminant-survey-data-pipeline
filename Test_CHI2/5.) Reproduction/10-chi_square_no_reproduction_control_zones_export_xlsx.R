# No reproduction control (No/Yes) vs vegetation and phytogeographic zones
# Inputs: data7.xlsx and zones.xlsx
# Output: chi_square_results_no_reproduction_control_zones.xlsx
...
DATA_FILE<-"data7.xlsx";ZONES_FILE<-"zones.xlsx";OUTPUT_FILE<-"chi_square_results_no_reproduction_control_zones.xlsx"
ALPHA<-0.05;B<-100000L;SEED<-20260828L
NO_REPRODUCTION_CONTROL_COL<-"5.) Reproduction/Quelles méthodes de contrôle de la reproduction utilisez-vous ? /3= Aucun contrôle/0=Non, 1=Oui"
...
if(!NO_REPRODUCTION_CONTROL_COL%in%names(raw))stop("No-reproduction-control column not found exactly in data7.xlsx");if(!COMMUNE_COL%in%names(raw))stop("Commune column not found exactly")
...
data<-raw|>transmute(no_reproduction_control_code=str_squish(.data[[NO_REPRODUCTION_CONTROL_COL]]),commune_original=str_squish(.data[[COMMUNE_COL]]),commune_key=normalize_key(.data[[COMMUNE_COL]]))|>mutate(no_reproduction_control=factor(no_reproduction_control_code,levels=c("0","1"),labels=c("No","Yes")),invalid_no_reproduction_control=is.na(no_reproduction_control)&!is.na(no_reproduction_control_code)&no_reproduction_control_code!="")|>left_join(zones,by="commune_key")
run_test<-function(df,zone_var,label){
 d<-df|>filter(!is.na(no_reproduction_control),!is.na(.data[[zone_var]]))|>droplevels();obs<-table(d$no_reproduction_control,d[[zone_var]]);pearson<-suppressWarnings(chisq.test(obs,correct=FALSE));exp<-pearson$expected;n<-sum(obs);cells<-length(exp);dfchi<-(nrow(obs)-1)*(ncol(obs)-1);n5<-sum(exp<5);n1<-sum(exp<1);pct5<-100*n5/cells;margins<-!any(rowSums(obs)==0)&!any(colSums(obs)==0);valid<-n1==0&pct5<=20&margins
...
veg<-run_test(data,"vegetation_zone","No reproduction control vs vegetation zone");phyto<-run_test(data,"phytogeo_zone","No reproduction control vs phytogeographic zone");summary<-bind_rows(veg$summary,phyto$summary)
matrix_df<-function(x){
  ...
  out<-data.frame(`No reproduction control`=rn,stringsAsFactors=FALSE,check.names=FALSE)
  for(j in seq_len(nc))out[[cn[j]]]<-unname(m[,j])
  out
}
quality<-data.frame(Parameter=c("Rows read","Valid starting-goat-breed-sahelian records","No","Yes","Missing responses","Invalid codes","Unmatched zone records"),Value=c(nrow(data),sum(!is.na(data$no_reproduction_control)),sum(data$no_reproduction_control=="No",na.rm=TRUE),sum(data$no_reproduction_control=="Yes",na.rm=TRUE),sum(is.na(data$no_reproduction_control_code)|data$no_reproduction_control_code==""),sum(data$invalid_no_reproduction_control),sum(is.na(data$vegetation_zone))))
notes<-data.frame(Parameter=c("Variable type","Code 0 interpretation","Null hypothesis","Expected-count rule","Monte Carlo","Multiple-response caution","Independence","Survey design"),Assessment=c("No reproduction control and zone are qualitative variables, so chi-square is appropriate.","The source label defines 0=No and 1=Yes.","No reproduction control status and zone are independent.","No expected count below 1 and no more than 20% below 5.","Monte Carlo with 100,000 simulations is exported as sensitivity analysis and selected only if expected-count conditions fail.","This is one binary modality of a multiple-response question on reproduction-control methods used in the herd. All valid livestock-unit responses are included. Analyze each control-method modality separately and adjust p-values when reporting several tests.","Each livestock unit contributes once to this no-reproduction-control table.","Use Rao-Scott chi-square if weights, strata, or village clusters apply."))
sheets<-list()
