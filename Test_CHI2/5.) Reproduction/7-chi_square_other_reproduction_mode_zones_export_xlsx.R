# Other reproduction mode (No/Yes) vs vegetation and phytogeographic zones
# Inputs: data7.xlsx and zones.xlsx
# Output: chi_square_results_other_reproduction_mode_zones.xlsx
...
DATA_FILE<-"data7.xlsx";ZONES_FILE<-"zones.xlsx";OUTPUT_FILE<-"chi_square_results_other_reproduction_mode_zones.xlsx"
ALPHA<-0.05;B<-100000L;SEED<-20260828L
OTHER_REPRODUCTION_MODE_COL<-"5.) Reproduction/Quel mode de reproduction adoptez-vous dans le troupeau ?  /3= autre/0=Non, 1=Oui"
...
if(!OTHER_REPRODUCTION_MODE_COL%in%names(raw))stop("Other reproduction-mode column not found exactly in data7.xlsx");if(!COMMUNE_COL%in%names(raw))stop("Commune column not found exactly")
...
data<-raw|>transmute(other_reproduction_mode_code=str_squish(.data[[OTHER_REPRODUCTION_MODE_COL]]),commune_original=str_squish(.data[[COMMUNE_COL]]),commune_key=normalize_key(.data[[COMMUNE_COL]]))|>mutate(other_reproduction_mode=factor(other_reproduction_mode_code,levels=c("0","1"),labels=c("No","Yes")),invalid_other_reproduction_mode=is.na(other_reproduction_mode)&!is.na(other_reproduction_mode_code)&other_reproduction_mode_code!="")|>left_join(zones,by="commune_key")
run_test<-function(df,zone_var,label){
 d<-df|>filter(!is.na(other_reproduction_mode),!is.na(.data[[zone_var]]))|>droplevels();obs<-table(d$other_reproduction_mode,d[[zone_var]]);pearson<-suppressWarnings(chisq.test(obs,correct=FALSE));exp<-pearson$expected;n<-sum(obs);cells<-length(exp);dfchi<-(nrow(obs)-1)*(ncol(obs)-1);n5<-sum(exp<5);n1<-sum(exp<1);pct5<-100*n5/cells;margins<-!any(rowSums(obs)==0)&!any(colSums(obs)==0);valid<-n1==0&pct5<=20&margins
...
veg<-run_test(data,"vegetation_zone","Other reproduction mode vs vegetation zone");phyto<-run_test(data,"phytogeo_zone","Other reproduction mode vs phytogeographic zone");summary<-bind_rows(veg$summary,phyto$summary)
