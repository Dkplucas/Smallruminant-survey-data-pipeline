# Marital status vs vegetation and phytogeographic zones
# Inputs: data7.xlsx and zones.xlsx
# Output: chi_square_results_marital_status_zones.xlsx
...
OUTPUT_FILE <- "chi_square_results_marital_status_zones.xlsx"
ALPHA <- 0.05
B <- 100000L
SEED <- 20260827L
MARITAL_STATUS_COL <- "I.- IDENTIFICATION DU CHEF DE MENAGE /Situation matrimoniale : 1=Celibataire, 2=Divorce(e), 3=Marie, 4=Veuf/ve"
...
if (!MARITAL_STATUS_COL %in% names(data_raw)) stop("Marital-status column not found exactly in data7.xlsx")
...
data <- data_raw |>
  transmute(
    marital_status_code=str_squish(.data[[MARITAL_STATUS_COL]]),
    commune_original=str_squish(.data[[COMMUNE_COL]]),
    commune_key=normalize_key(.data[[COMMUNE_COL]])
  ) |>
  mutate(
    marital_status=factor(marital_status_code, levels=c("1","2","3","4"),
                        labels=c("Single","Divorced","Married","Widowed")),
    invalid_marital_status=is.na(marital_status) & !is.na(marital_status_code) & marital_status_code!=""
  ) |>
  left_join(zones, by="commune_key")

run_test <- function(df, zone_var, label) {
  d <- df |> filter(!is.na(marital_status), !is.na(.data[[zone_var]])) |> droplevels()
  observed <- table(d$marital_status, d[[zone_var]])
...
veg <- run_test(data,"vegetation_zone","Marital status vs vegetation zone")
phyto <- run_test(data,"phytogeo_zone","Marital status vs phytogeographic zone")
summary <- bind_rows(veg$summary,phyto$summary)

matrix_df <- function(x) {
  z <- as.data.frame.matrix(x)
  data.frame(`Marital status`=rownames(z),z,check.names=FALSE,row.names=NULL)
}

quality <- data.frame(
  Parameter=c("Rows read","Valid marital-status records","Missing marital-status records",
              "Invalid marital-status codes","Unmatched zone records"),
  Value=c(nrow(data),sum(!is.na(data$marital_status)),
          sum(is.na(data$marital_status_code)|data$marital_status_code==""),
          sum(data$invalid_marital_status),sum(is.na(data$vegetation_zone))))
...
notes <- data.frame(
  ...
  Assessment=c(
    "Marital status and zone are qualitative variables, so a chi-square test of independence is appropriate.",
    "Marital status and zone are independent.",
