# Fodder storage as an environmental adaptation strategy vs zones
# Inputs: data7.xlsx and zones.xlsx
# Output: chi_square_results_environmental_adaptation_fodder_storage_zones.xlsx
...
OUTPUT_FILE <- "chi_square_results_environmental_adaptation_fodder_storage_zones.xlsx"
ALPHA <- 0.05
B <- 100000L
SEED <- 20260914L
CHILD_COL <- "10.) Contexte environnemental/Quelles stratégies avez-vous mises en place pour adapter votre élevage aux contraintes environnementales ? /3= Stockage de fourrages"
...
resolve_column <- function(expected,headers,terms,label){
  ...
  cand <- headers[str_detect(nh,"contexteenvironnemental|influence.*croisement|stockagedefourrages")]
...
CHILD_COL <- resolve_column(CHILD_COL,names(raw),c("Contexte environnemental","stratégies","contraintes environnementales","3 Stockage de fourrages"),"Fodder storage")
...
data <- raw |>
  ...
  mutate(eligible=TRUE,invalid_child=!is.na(child_code)&child_code!=""&!child_code%in%c("0","1"),fodder_storage=factor(ifelse(child_code%in%c("0","1"),child_code,NA_character_),levels=c("0","1"),labels=c("No","Yes")))
run_test <- function(zone_var,label){
  d <- data |> filter(eligible,!is.na(fodder_storage),!is.na(.data[[zone_var]]),.data[[zone_var]]!="") |> droplevels()
  obs <- table(d$fodder_storage,d[[zone_var]]); n<-sum(obs); nr<-nrow(obs); nc<-ncol(obs); dfchi<-(nr-1)*(nc-1)
  if(n==0L||nr<2L||nc<2L){
