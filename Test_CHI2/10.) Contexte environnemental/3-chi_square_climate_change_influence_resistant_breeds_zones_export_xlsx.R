# Adoption of more resistant breeds as a climate-change influence on crossbreeding practices vs zones
# Inputs: data7.xlsx and zones.xlsx
# Output: chi_square_results_climate_change_influence_resistant_breeds_zones.xlsx
...
OUTPUT_FILE <- "chi_square_results_climate_change_influence_resistant_breeds_zones.xlsx"
ALPHA <- 0.05
B <- 100000L
SEED <- 20260914L
CHILD_COL <- "10.) Contexte environnemental/Comment ces changements ont-ils influencé vos pratiques de croisement ? /1= Adoption de races plus résistantes"
...
resolve_column <- function(expected,headers,terms,label){
  ...
  cand <- headers[str_detect(nh,"contexteenvironnemental|influence.*croisement|racesplusresistantes")]
...
CHILD_COL <- resolve_column(CHILD_COL,names(raw),c("Contexte environnemental","influencé","pratiques de croisement","1 Adoption de races plus résistantes"),"Resistant-breed adoption")
...
data <- raw |>
  ...
  mutate(eligible=parent_code=="1",invalid_child=!is.na(child_code)&child_code!=""&!child_code%in%c("0","1"),invalid_parent=!is.na(parent_code)&parent_code!=""&!parent_code%in%c("0","1","2"),ineligible_positive=!eligible&child_code=="1",resistant_breed_adoption=factor(ifelse(child_code%in%c("0","1"),child_code,NA_character_),levels=c("0","1"),labels=c("No","Yes")))
run_test <- function(zone_var,label){
  d <- data |> filter(eligible,!is.na(resistant_breed_adoption),!is.na(.data[[zone_var]]),.data[[zone_var]]!="") |> droplevels()
  obs <- table(d$resistant_breed_adoption,d[[zone_var]]); n<-sum(obs); nr<-nrow(obs); nc<-ncol(obs); dfchi<-(nr-1)*(nc-1)
  if(n==0L||nr<2L||nc<2L){
