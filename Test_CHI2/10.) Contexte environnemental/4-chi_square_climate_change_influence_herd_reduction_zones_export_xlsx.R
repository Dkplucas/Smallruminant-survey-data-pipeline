# Herd reduction as a climate-change influence on crossbreeding practices vs zones
# Inputs: data7.xlsx and zones.xlsx
# Output: chi_square_results_climate_change_influence_herd_reduction_zones.xlsx
...
OUTPUT_FILE <- "chi_square_results_climate_change_influence_herd_reduction_zones.xlsx"
ALPHA <- 0.05
B <- 100000L
SEED <- 20260914L
CHILD_COL <- "10.) Contexte environnemental/Comment ces changements ont-ils influencé vos pratiques de croisement ? /2= Réduction du troupeau"
...
resolve_column <- function(expected,headers,terms,label){
  ...
  cand <- headers[str_detect(nh,"contexteenvironnemental|influence.*croisement|reductiondutroupeau")]
...
CHILD_COL <- resolve_column(CHILD_COL,names(raw),c("Contexte environnemental","influencé","pratiques de croisement","2 Réduction du troupeau"),"Herd reduction")
...
data <- raw |>
  ...
  mutate(eligible=parent_code=="1",invalid_child=!is.na(child_code)&child_code!=""&!child_code%in%c("0","1"),invalid_parent=!is.na(parent_code)&parent_code!=""&!parent_code%in%c("0","1","2"),ineligible_positive=!eligible&child_code=="1",herd_reduction=factor(ifelse(child_code%in%c("0","1"),child_code,NA_character_),levels=c("0","1"),labels=c("No","Yes")))
run_test <- function(zone_var,label){
  d <- data |> filter(eligible,!is.na(herd_reduction),!is.na(.data[[zone_var]]),.data[[zone_var]]!="") |> droplevels()
  obs <- table(d$herd_reduction,d[[zone_var]]); n<-sum(obs); nr<-nrow(obs); nc<-ncol(obs); dfchi<-(nr-1)*(nc-1)
  if(n==0L||nr<2L||nc<2L){
