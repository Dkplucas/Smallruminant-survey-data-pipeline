# Modification of reproduction periods as a climate-change influence on crossbreeding practices vs zones
# Inputs: data7.xlsx and zones.xlsx
# Output: chi_square_results_climate_change_influence_reproduction_periods_zones.xlsx
...
OUTPUT_FILE <- "chi_square_results_climate_change_influence_reproduction_periods_zones.xlsx"
ALPHA <- 0.05
B <- 100000L
SEED <- 20260914L
CHILD_COL <- "10.) Contexte environnemental/Comment ces changements ont-ils influencé vos pratiques de croisement ? /3= Modification des périodes de reproduction"
PARENT_COL <- "10.) Contexte environnemental/Avez-vous observé des changements climatiques affectant votre élevage ?: 0=Ne sait pas, 1=Oui, 2=Non"
...
resolve_column <- function(expected,headers,terms,label){
  ...
  cand <- headers[str_detect(nh,"contexteenvironnemental|influence.*croisement|modificationdesperiodesdereproduction")]
...
CHILD_COL <- resolve_column(CHILD_COL,names(raw),c("Contexte environnemental","influencé","pratiques de croisement","3 Modification des périodes de reproduction"),"Modification of reproduction periods")
PARENT_COL <- resolve_column(PARENT_COL,names(raw),c("Contexte environnemental","changements climatiques","affectant votre élevage","Ne sait pas","Oui","Non"),"Parent climate-change response")
...
data <- raw |>
  transmute(row_id=row_number(),child_code=str_squish(.data[[CHILD_COL]]),parent_code=str_squish(.data[[PARENT_COL]]),commune_raw=str_squish(.data[[COMMUNE_COL]]),commune_key=normalize_key(.data[[COMMUNE_COL]])) |>
  left_join(zones,by="commune_key") |>
  mutate(eligible=parent_code=="1",invalid_child=!is.na(child_code)&child_code!=""&!child_code%in%c("0","1"),invalid_parent=!is.na(parent_code)&parent_code!=""&!parent_code%in%c("0","1","2"),ineligible_positive=!eligible&child_code=="1",reproduction_period_modification=factor(ifelse(child_code%in%c("0","1"),child_code,NA_character_),levels=c("0","1"),labels=c("No","Yes")))
run_test <- function(zone_var,label){
  d <- data |> filter(eligible,!is.na(reproduction_period_modification),!is.na(.data[[zone_var]]),.data[[zone_var]]!="") |> droplevels()
  obs <- table(d$reproduction_period_modification,d[[zone_var]]); n<-sum(obs); nr<-nrow(obs); nc<-ncol(obs); dfchi<-(nr-1)*(nc-1)
  if(n==0L||nr<2L||nc<2L){
...
veg<-run_test("vegetation_zone","Modification of reproduction periods x vegetation zone")
phyto<-run_test("phytogeo_zone","Modification of reproduction periods x phytogeographic zone")
quality<-data.frame(Metric=c("Total records","Eligible parent Yes (1)","Valid eligible child responses","No (0)","Yes (1)","Blank child responses","Invalid child codes","Invalid parent codes","Ineligible positive child responses","Unmatched communes"),Value=c(nrow(data),sum(data$eligible,na.rm=TRUE),sum(data$eligible&!is.na(data$reproduction_period_modification),na.rm=TRUE),sum(data$eligible&data$child_code=="0",na.rm=TRUE),sum(data$eligible&data$child_code=="1",na.rm=TRUE),sum(is.na(data$child_code)|data$child_code==""),sum(data$invalid_child),sum(data$invalid_parent),sum(data$ineligible_positive,na.rm=TRUE),sum(is.na(data$vegetation_zone))))
notes<-data.frame(Parameter=c("Variable type","Eligibility","Coding","Null hypothesis","Expected-count rule","Monte Carlo","Effect size"),Assessment=c("Conditional binary qualitative modality.","Only parent code 1 (Yes, climate changes observed) is eligible.","0=option not selected; 1=modification of reproduction periods selected.","Selection of modification of reproduction periods and geographical zone are independent.","No expected count below 1 and no more than 20% below 5.","100,000 simulations, selected when Pearson assumptions fail.","Cramer's V."),stringsAsFactors=FALSE)
