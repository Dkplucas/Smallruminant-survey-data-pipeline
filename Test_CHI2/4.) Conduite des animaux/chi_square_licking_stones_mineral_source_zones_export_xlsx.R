# Licking stones as mineral source (No/Yes) vs vegetation and phytogeographic zones
# Inputs: data7.xlsx and zones.xlsx
# Output: chi_square_results_licking_stones_mineral_source_zones.xlsx
...
DATA_FILE<-"data7.xlsx";ZONES_FILE<-"zones.xlsx";OUTPUT_FILE<-"chi_square_results_licking_stones_mineral_source_zones.xlsx"
ALPHA<-0.05;B<-100000L;SEED<-20260828L
LICKING_STONES_MINERAL_SOURCE_COL<-"4.) Conduite des animaux/Si oui, quelles sources de minéraux utilisez-vous ? /2= pierres à lécher/0=Non, 1=Oui"
PARENT_MINERAL_COL<-"4.) Conduite des animaux/Faîtes-vous le complément minéral ? 0=Non, 1=Oui"
...
if(!LICKING_STONES_MINERAL_SOURCE_COL%in%names(raw))stop("Licking-stones mineral-source column not found exactly in data7.xlsx");if(!PARENT_MINERAL_COL%in%names(raw))stop("Parent mineral-supplementation column not found exactly in data7.xlsx");if(!COMMUNE_COL%in%names(raw))stop("Commune column not found exactly")
...
data<-raw|>transmute(parent_mineral=str_squish(.data[[PARENT_MINERAL_COL]]),licking_stones_mineral_source_code=str_squish(.data[[LICKING_STONES_MINERAL_SOURCE_COL]]),commune_original=str_squish(.data[[COMMUNE_COL]]),commune_key=normalize_key(.data[[COMMUNE_COL]]))|>mutate(eligible=parent_mineral=="1",licking_stones_mineral_source=factor(licking_stones_mineral_source_code,levels=c("0","1"),labels=c("No","Yes")),invalid_licking_stones_mineral_source=eligible&is.na(licking_stones_mineral_source)&!is.na(licking_stones_mineral_source_code)&licking_stones_mineral_source_code!="",ineligible_positive=parent_mineral=="0"&licking_stones_mineral_source_code=="1")|>left_join(zones,by="commune_key")
run_test<-function(df,zone_var,label){
 d<-df|>filter(eligible,!is.na(licking_stones_mineral_source),!is.na(.data[[zone_var]]))|>droplevels();obs<-table(d$licking_stones_mineral_source,d[[zone_var]]);pearson<-suppressWarnings(chisq.test(obs,correct=FALSE));exp<-pearson$expected;n<-sum(obs);cells<-length(exp);dfchi<-(nrow(obs)-1)*(ncol(obs)-1);n5<-sum(exp<5);n1<-sum(exp<1);pct5<-100*n5/cells;margins<-!any(rowSums(obs)==0)&!any(colSums(obs)==0);valid<-n1==0&pct5<=20&margins
...
veg<-run_test(data,"vegetation_zone","Licking stones as mineral source vs vegetation zone");phyto<-run_test(data,"phytogeo_zone","Licking stones as mineral source vs phytogeographic zone");summary<-bind_rows(veg$summary,phyto$summary)
matrix_df<-function(x){
  ...
  out<-data.frame(`Licking stones as mineral source`=rn,stringsAsFactors=FALSE,check.names=FALSE)
  for(j in seq_len(nc))out[[cn[j]]]<-unname(m[,j])
  out
}
quality<-data.frame(Parameter=c("Rows read","Eligible units providing mineral supplementation","Eligible valid market-gardening by-product records","Eligible No","Eligible Yes","Eligible missing responses","Eligible invalid codes","Ineligible positive responses","Unmatched eligible zone records"),Value=c(nrow(data),sum(data$eligible),sum(data$eligible&!is.na(data$licking_stones_mineral_source)),sum(data$eligible&data$licking_stones_mineral_source=="No",na.rm=TRUE),sum(data$eligible&data$licking_stones_mineral_source=="Yes",na.rm=TRUE),sum(data$eligible&(is.na(data$licking_stones_mineral_source_code)|data$licking_stones_mineral_source_code=="")),sum(data$invalid_licking_stones_mineral_source),sum(data$ineligible_positive),sum(data$eligible&is.na(data$vegetation_zone))))
notes<-data.frame(Parameter=c("Variable type","Code 0 interpretation","Null hypothesis","Expected-count rule","Monte Carlo","Multiple-response caution","Independence","Survey design"),Assessment=c("Licking stones as mineral source and zone are qualitative variables, so chi-square is appropriate.","The source label defines 0=No and 1=Yes.","Licking stones as mineral source status and zone are independent.","No expected count below 1 and no more than 20% below 5.","Monte Carlo with 100,000 simulations is exported as sensitivity analysis and selected only if expected-count conditions fail.","This is a conditional modality of a multiple-response mineral-source question. Analysis is restricted to livestock units providing mineral supplementation. Analyze each mineral source separately and adjust p-values if several are tested.","Among eligible livestock units, each unit contributes once to the licking-stones table.","Use Rao-Scott chi-square if weights, strata, or village clusters apply."))
sheets<-list()
