# Market-gardening by-products as supplements (No/Yes) vs vegetation and phytogeographic zones
# Inputs: data7.xlsx and zones.xlsx
# Output: chi_square_results_market_gardening_byproducts_supplement_zones.xlsx
...
DATA_FILE<-"data7.xlsx";ZONES_FILE<-"zones.xlsx";OUTPUT_FILE<-"chi_square_results_market_gardening_byproducts_supplement_zones.xlsx"
ALPHA<-0.05;B<-100000L;SEED<-20260828L
MARKET_GARDENING_BYPRODUCTS_COL<-"4.) Conduite des animaux/Si oui, quels sont ces compléments alimentaires ? /1= sous-produits maraîchers/0=Non, 1=Oui"
...
if(!MARKET_GARDENING_BYPRODUCTS_COL%in%names(raw))stop("Market-gardening-by-products column not found exactly in data7.xlsx");if(!PARENT_SUPPLEMENTATION_COL%in%names(raw))stop("Parent pasture-conduct column not found exactly in data7.xlsx");if(!COMMUNE_COL%in%names(raw))stop("Commune column not found exactly")
...
data<-raw|>transmute(parent_supplementation=str_squish(.data[[PARENT_SUPPLEMENTATION_COL]]),market_gardening_byproducts_code=str_squish(.data[[MARKET_GARDENING_BYPRODUCTS_COL]]),commune_original=str_squish(.data[[COMMUNE_COL]]),commune_key=normalize_key(.data[[COMMUNE_COL]]))|>mutate(eligible=parent_supplementation=="1",market_gardening_byproducts=factor(market_gardening_byproducts_code,levels=c("0","1"),labels=c("No","Yes")),invalid_market_gardening_byproducts=eligible&is.na(market_gardening_byproducts)&!is.na(market_gardening_byproducts_code)&market_gardening_byproducts_code!="",ineligible_positive=parent_supplementation=="0"&market_gardening_byproducts_code=="1")|>left_join(zones,by="commune_key")
run_test<-function(df,zone_var,label){
 d<-df|>filter(eligible,!is.na(market_gardening_byproducts),!is.na(.data[[zone_var]]))|>droplevels();obs<-table(d$market_gardening_byproducts,d[[zone_var]]);pearson<-suppressWarnings(chisq.test(obs,correct=FALSE));exp<-pearson$expected;n<-sum(obs);cells<-length(exp);dfchi<-(nrow(obs)-1)*(ncol(obs)-1);n5<-sum(exp<5);n1<-sum(exp<1);pct5<-100*n5/cells;margins<-!any(rowSums(obs)==0)&!any(colSums(obs)==0);valid<-n1==0&pct5<=20&margins
...
veg<-run_test(data,"vegetation_zone","Market-gardening by-products as supplements vs vegetation zone");phyto<-run_test(data,"phytogeo_zone","Market-gardening by-products as supplements vs phytogeographic zone");summary<-bind_rows(veg$summary,phyto$summary)
matrix_df<-function(x){
  ...
  out<-data.frame(`Market-gardening by-products as supplements`=rn,stringsAsFactors=FALSE,check.names=FALSE)
  for(j in seq_len(nc))out[[cn[j]]]<-unname(m[,j])
  out
}
quality<-data.frame(Parameter=c("Rows read","Eligible units receiving post-pasture supplements","Eligible valid market-gardening by-product records","Eligible No","Eligible Yes","Eligible missing responses","Eligible invalid codes","Ineligible positive responses","Unmatched eligible zone records"),Value=c(nrow(data),sum(data$eligible),sum(data$eligible&!is.na(data$market_gardening_byproducts)),sum(data$eligible&data$market_gardening_byproducts=="No",na.rm=TRUE),sum(data$eligible&data$market_gardening_byproducts=="Yes",na.rm=TRUE),sum(data$eligible&(is.na(data$market_gardening_byproducts_code)|data$market_gardening_byproducts_code=="")),sum(data$invalid_market_gardening_byproducts),sum(data$ineligible_positive),sum(data$eligible&is.na(data$vegetation_zone))))
notes<-data.frame(Parameter=c("Variable type","Code 0 interpretation","Null hypothesis","Expected-count rule","Monte Carlo","Multiple-response caution","Independence","Survey design"),Assessment=c("Market-gardening by-products as supplements and zone are qualitative variables, so chi-square is appropriate.","The source label defines 0=No and 1=Yes.","Market-gardening by-products as supplements status and zone are independent.","No expected count below 1 and no more than 20% below 5.","Monte Carlo with 100,000 simulations is exported as sensitivity analysis and selected only if expected-count conditions fail.","This is a conditional modality of a multiple-response supplement-type question. Analysis is restricted to livestock units receiving post-pasture supplements. Analyze each supplement type separately and adjust p-values if several are tested.","Among eligible livestock units, each unit contributes once to the market-gardening-by-products table.","Use Rao-Scott chi-square if weights, strata, or village clusters apply."))
sheets<-list()
